...
- 将 Windows、Linux 和 macOS 的原生应用菜单改为中文，包含文件、编辑、视图、窗口和帮助菜单。
- 将 GitHub README 与扩展主题说明改为中文优先的中英双语文档。
- 修复网站主题加载器对 WebP 预览图和主题图片的支持，完整展示 61 套内置主题。
- 完成 macOS Apple 芯片与 Intel 双架构自动构建和发布。
- 配置 Apple Developer 凭据时生成签名并公证的安装包；未配置时自动发布未签名预览版。

### English

- Localized the native Windows, Linux, and macOS application menus into Chinese.
- Reworked the GitHub README and expanded-theme guide as Chinese-English bilingual documents.
- Fixed the website theme loader to support WebP previews and artwork across all 61 bundled themes.
- Completed Apple Silicon and Intel macOS release jobs.
- Signed and notarized packages are produced when Apple credentials are configured; otherwise the workflow publishes unsigned preview packages.
